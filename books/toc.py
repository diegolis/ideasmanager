...
class TocTreeprocessor(Treeprocessor):
    ...
    def build_toc_div(self, toc_list):
        ...
        def build_etree_ul(toc_list, parent, levels, level=0):
            ...
            for item in toc_list:
                ...
                link.text = "{}: {}".format(levels[level], item.get('name', ''))
                link.attrib["href"] = '#' + item.get('id', '')
                if item['children']:
                    build_etree_ul(item['children'], li, levels, level+1)
            return ul

        build_etree_ul(toc_list, div, levels)
